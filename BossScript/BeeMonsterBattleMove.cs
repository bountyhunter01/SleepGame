...
public enum BeeBossState { MoveToAppearPoint = 0, Phase01, Phase02 }
public class BeeMonsterBattleMove : MonoBehaviour
{
    [SerializeField]
    ...
    private IEnumerator Phase01()
    {
        ...
        while (true)
        {
            ...
            // 보스의 체력 게이지에 따라 상태 전환
            if (bossGauge.currentGauge >= bossGauge.bossMaxGauge * 0.7f)
            {
                attackPatten.StopFiring(AttackType.SingleFire);
                ChangState(BossState.Phase02);
                break; // 상태 전환시 현재 코루틴 중지
            }
            yield return null;
        }
    }
    ...
    {
        attackPatten.StartFiring(AttackType.CircleFire);
        ...
        while (true)
        {
            ...
            {
                direction = new Vector3(transform.position.x * -1, -0.1f);
            }
            else if (transform.position.x >= stageData.LimitMax.x)
            {
                direction = new Vector3(transform.position.x * -1, 0.1f);
